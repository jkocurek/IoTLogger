...
class SensorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create sensor" do
    assert_difference('Sensor.count') do
      post sensors_url, params: { sensor: { name: @sensor.name, status: @sensor.status, temp: @sensor.temp } }
    end
  ...
  test "should update sensor" do
    patch sensor_url(@sensor), params: { sensor: { name: @sensor.name, status: @sensor.status, temp: @sensor.temp } }
